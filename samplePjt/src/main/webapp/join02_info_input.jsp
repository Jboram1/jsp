...
<html>
	<head>
		<c:if test="${param.f_agree!='agree' }">
			<script>
				alert("약관동의 없이 회원가입은 진행할 수 없습니다.");
		...
		<script>
			$(function(){
				...
				$("#insertBtn").click(function(){
					...
					//var idPtn = /^[a-z]{2,5}$/; //영문소문자 2~5자리 사이 값을 비교 패턴 (/^ - 시작표시, [a-z]-a부터z까지, {2,5}-2자리부터 5자리까지, $/-마침표시)
					//var idPtn = /^[a-z]{1}[a-zA-Z0-9]{3,16}$/; //첫1글자 영소문자이면서 다음 3자리에서 16자리는 영문,숫자가능
					var idPtn = /^[a-zA-Z0-9]{2,10}$/; //
					//var namePtn = /^[가-힣]{1,3}$/; //국문 1~3자리까지 가능
					//var pwPtn = /^(?=.*[a-zA-Z])(?=.*[0-9])(?=.*[~!@#$%^&]).{3,}$/; //{8,}모두 포함 8자리까지. 영문자특수문자숫자1개이상
					
					//var f_tellPtn = /^[0-9]{2,3}$/; // 유선전화가능 국번 02,043,010
					//var m_tellPtn = /^[0-9]{3,4}$/; // 415,1111 (3~4자리)
					if(!idPtn.test(id)){
				...
				$("#idChkBtn").click(function(){
					...
					$.ajax({
						...
						type:"post",
						data:{"id":id},
						dataType:"text", //text,json,xml //utf-8설정해야함
