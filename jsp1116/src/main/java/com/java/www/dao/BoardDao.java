	...
	public ArrayList<BoardDto> bListSelectAll() {
		try {
			...
			while(rs.next()) {
				//데이터 가져오기
				bno = rs.getInt("bno");
				//System.out.println("bListSelectAll :"+bno); //오류찾기
				...
				list.add(new BoardDto(bno,btitle,bcontent,bdate,id,bgroup,bgroup,bindent,bhit,bfile));
				//BLservice에 객체선언 후 arraylist에 담아준다
			}
		} catch (Exception e) {
			e.printStackTrace();
		}finally {
